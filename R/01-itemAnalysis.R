...
"
  ...
  ui <- fluidPage(
    ...
    sidebarPanel(width = 4,
                 ...
                 conditionalPanel(
                   ...
                   br(),
                   br(),
                   div(h3(id="ab", "Select Data Type"), align = "left"),
                   tags$h3(tags$style(
                       "#ab{
                       color: darkblue;
                       font-size: 20px;
                 ...
                 ## CONDITIONAL PANEL 4 - Distractor Analysis ##
                 ...
                 conditionalPanel(
                   ...
                   selectInput("distGraph", h3(id="ab", "Select Graph Type"),
                 ...
                 ## CONDITIONAL PANEL 6 - Test Statistics ##
                 ...
                 conditionalPanel(
                   ...
                   ######################################################################################
                   #imageOutput("image6",width = "75%", height = "100px", inline = TRUE),
                   ######################################################################################
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        ## M.PANEL 4 - Distractor Analysis ##

        # tabPanel(
        #
        #   h4(id="a", "DISTRACTOR ANALYSIS"),
        #   value = 3,
        #
        #   br(),
        #
        ...
        ## M.PANEL 6 - Test Statistics ##

        tabPanel(
          h4(id="a","TEST STATISTICS"),
          value = 5,
          br(),
          textOutput("text4"),
          tags$head(
  ...
  server <- function(input, output, session) {
    ...
    data <- reactive({
      dat <- input$data1
      if (is.null(dat)){
        return(paste0("Upload Data"))
      } else if (input$type2==1){
        if(tools::file_ext(dat$datapath) != "csv"){
          data.frame(Warning = "PLEASE SELECT THE CORRECT FILE FORMAT")
        } else {
          read.csv2(dat$datapath, header=isTRUE(input$header), sep = ";")
        }
        ...
        if(tools::file_ext(dat$datapath) != "sav"){
          data.frame(uyari = "PLEASE SELECT THE CORRECT FILE FORMAT")
        } else {
          read.spss(dat$datapath, to.data.frame = TRUE, use.value.labels = FALSE)
        }
      }
    })
    ...
    output$table1 <- render_gt(
      align = "center", {
        ...
        itemAnalysis1 <- function(data, key){
    ...
    datARem<-reactive({
      ...
      if(input$type == 1){
        datA_Rem <- datA()[, ITEMAN_ENV$remain_items]  }
      else
      {
        datA_Rem <- datA()[, ITEMAN_ENV$remain_items] }
    })
    ...
    observeEvent(input$omit, {
      ...
      output$buton <- render_gt(align="center", {
        ...
        itemAnalysis1_2 <- function(data, key) {
        ...
        if (input$type == 2) {
          ...
          res1 <- itemAnalysis2_2(data = datARem())
        } else {
          res1 <- itemAnalysis1_2(datARem(), keyARem())
        }

        res1 <- res1[-1]
        res1 <- cbind(ITEMAN_ENV$remain_items, res1)
        ...
        res1<- res1 %>%
          ...
          tab_style(
            ...
            locations = cells_body(
              columns = c(ITEMAN_ENV$remain_items)
        ...
        res1<- res1 %>%
          ...
          tab_style(
            style = cell_text(weight = "bolder",color="red",
                              decorate="underline",stretch="extra-expanded"),
            locations = cells_body(
              columns = c(Item_Difficulty),
              rows =Item_Difficulty >= 0.80 | Item_Difficulty <= 0.25
    ...
    #     # remain_items <- setdiff(all, omit)
    #     #
    #     # colnames(datAGL) <- rnamesGL
    #     # colnames(keyAGL) <- rnamesGL
    #
    #     # datAKalanGL <- datAGL[, remain_items]
    #     # keyAKalanGL <- keyAGL[, remain_items]
    #
    #
    #     if (is.null(input$data1)) {
    #       return(NULL)
    ...
    #         liste <- list()
    #
    #         a <- length(unique(as.factor(y)))
    #
    #         for (i in 1:ncol(x))  {
    #           liste[[i]] <- prop.table(table(x[, i]))
    #         }
    #
    #         boss <- matrix(NA, ncol(x), a)
    #
    #         boss<-as.data.frame(boss)
    #
    #         colnames(boss) <- LETTERS[1:a]
    #
    #         satis <- character()
    #         for (i in 1:ncol(x)) {
    #           satis[i] <- paste0("item", i)
    #         }
    #
    #         for (i in 1:ncol(x)) {
    #           boss[i, ] <- liste[[i]]
    ...
    #         boss <- round(boss, 3)
    #
    #         boss$ItemS<-remain_items
    #
    #         boss<-data.frame(Items= boss$ItemS,boss[,1:a])
    #
    #
    #         if (a==5) {
    #
    ...
    #
    #
    #           boss<- boss %>% tab_header(
    #             title= md("**DISTRACTOR ANALYSIS RESULTS**")
    #           )
    #
    #           boss<- boss %>%
    #
    ...
    #                                 color="black",stretch="extra-expanded"),
    #               locations = cells_body(
    #                 columns = c(Items)
    #               ))
    #
    ...
    #
    #             tab_style(
    #               style = cell_text(weight = "bolder",color="black",
    #                                 stretch="extra-expanded"),
    #               locations = cells_body(
    #                 columns = c(Items)
    #               ))
    ...
    #       if (input$type == 2) {
    #         return (
    #           print("DISTRACTOR ANALYSIS CANNOT BE PERFORMED
    #             FOR ITEMS SCORED AS 1-0!"))
    #       } else
    #
    #       {
    #         resultDistAnalysis <- sec(x = datARem(), y = keyARem()) }
    #     }
    #   }
    # })
    ...
    output$print<-render_gt({
      ...
      output$dlItemAn <- downloadHandler(
        ...
        if (is.null(input$itm)) {
          ...
          content = function(file) {
            write.csv2(ITEMAN_ENV$itemResult[["_data"]], file) } } else

            {  content = function(file) {
              write.csv2(ITEMAN_ENV$RemovedItemResult[["_data"]], file)  }
      ...
      output$downloadTestIst <- downloadHandler(
        ...
        content = function(file) {
          write.csv2(cbind(ITEMAN_ENV$saveist.1, ITEMAN_ENV$saveist.2, ITEMAN_ENV$saveist.3), file)
        }
      )

      # output$downloadDistAn <- downloadHandler(
      #
      #   filename = function() {
      #     "distractor_analysis.csv"
      #   },
      #   content = function(file) {
      #     write.csv2(resultDistAnalysis[["_data"]], file)
      #   }
      # )
    ...
    output$uiHeader <- renderUI({
      ...
      if(input$type2 == 3){
        ...
        checkboxInput("header", "The first line is the variable name?"
                      , value = TRUE)}
    })
    ...
    output$dlDistractor <- renderUI({
      ...
      fluidRow(
        column(
          ...
          # downloadButton(
          #   "downloadDistAn",
          #   h1(id="b","DOWNLOAD DISTRACTOR ANALYSIS")
          # )
        ),
        column(
          8,
    ...
    A <- B <- Biserial <-  Item_Difficulty <- Items <- Point_Biserial <- NULL
     resultDistAnalysis  <- saveist.1 <- NULL
    saveist.2 <- saveist.3 <- NULL
     RemovedItemResult <- itemResult <- NULL
  }
